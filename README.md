...
### v1.42
- WiFi信道频谱新增 AP 详情弹窗：点击热点列表任一行，显示信号强度（含信号条与等级配色）、频率/信道/估算距离、占用频率范围、频段、带宽、标准、加密方式与完整加密能力串
- 厂商信息由 ip.ustc.edu.cn 按 MAC 前 6 位（OUI）查询，AP 名字/楼宇/厂商在弹窗中异步补上
- 修正绑定块定位：80/160MHz 的主信道可在块内任意位置（如 149-161 块的主信道可为 157），块位置改由 ScanResult.centerFreq0 决定
- 频段行同时显示覆盖信道跨度与中心信道，如「信道 36 → 48（中心 42）」
- 频谱图 2.4G 曲线改回以主信道为中心的对称钟形：2.4G 相邻信道本就互相重叠，且 ScanResult 不报告 HT40 扩展方向，按块中心绘制会偏移
