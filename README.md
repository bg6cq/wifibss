...
- **WiFi信道频谱**：频谱曲线图展示周围所有 AP 的信道占用情况，支持 2.4G/5G 切换；热点列表可查看每个 AP 的强度/频率/信道/带宽/标准，名称列可在 MAC 与 AP 名字间切换，支持「聚合显示」：AP名模式按 AP 名合并同名不同 MAC 的 AP，MAC 模式按 MAC 合并
...
### v1.41
- 热点列表「标准」列由 4/5/6/7 改为带圈数字 ④⑤⑥⑦（Wi-Fi 4/5/6/7，6E 归 ⑥）
- 该列表头由「标」改为「标准」，列宽保持不变

### v1.40
- 「信道利用率」更名为「WiFi信道频谱」
- 聚合显示增强：按 AP名 显示时，同名的不同 BSSMAC（如 eduroam/ustcnet/ustc-guest 各自独立 BSSID）合并显示为一行，名称以 SSID 用 "/" 连接；按 MAC 显示时不同 MAC 不合并
- 5G/6G 宽信道（40/80/160MHz）按真实频段绘制——主信道位于频段最低端，弧顶在主信道、向右覆盖绑定的信道，不再以主信道为中心对称显示
- 2.4G 横轴左边界左移，信道 1 左侧留空 4 格，与 40MHz 曲线占宽对称；5G 分段边缘各留 20MHz 空档

### v1.39
- 信道利用率界面热点列表新增「聚合显示」：同一 BSSID 开通多个 SSID（如 eduroam/ustcnet/ustc-guest）时合并显示为一行，名称以 SSID 用 "/" 连接
- 修复部分 AP 名字未查询、只显示 MAC 的问题：查询失败的 BSSID 在 10 分钟内不再反复占用每轮的远程查询配额，其余 AP 可轮到查询

### v1.38
- 修复冷启动被误计为一次 AP 切换的问题：切换检测基线（最近一次 BSSID）改为持久化保存，下次启动恢复
- 修复同一个 AP 被重复计入「AP 切换次数」的问题：仅把合法的 12 位 BSSID 视为已连接，基线未恢复前不判定切换
- 修复信道利用界面扫描提示文字被并发查询覆盖的问题：提示计数改为按当前状态现算
- 修复信道利用界面列表在信号波动导致重排后行内容错位的问题
- 自动查询开关改为在协程启动前读取，避免读到过期设置

### v1.37
- 新增「信道利用」界面（主菜单进入）：信道频谱曲线图 + 周围热点列表
- 支持 2.4G/5G 频段切换，频谱图按信道宽度绘制钟形曲线，当前连接的 AP 以绿色高亮
- 热点列表显示名称/设备/强度/频率/信道/带宽/标准
- 名称列可在 MAC 与 AP 名字间切换，优先查本地 BSSMAC 库，未命中再按需查询远程 API
- 抽出 `WifiUtils.wifiStandardToString()`，主界面与新界面共用 WiFi 标准映射

### v1.36
- 界面文字精简：排序方式→排序，所在楼→楼，AP名字→AP名
- 修复 BSSMAC 信息编辑后列表不刷新的问题
- 修复 WiFi 断开时附近 AP 信息未清空的问题
- RSSI 图表下显示 AP 名称+信道号，并排除当前连接的 AP
- RSSI 图表 Y 轴从 -90 开始，低于 -90 的值不再超出图表区域
- 新增英语界面支持
...
### v1.33
- 适配手机暗主题（颜色资源、MaterialAlertDialog）
- 禁止横屏，锁定竖屏显示
- 根据BSSMAC查名字统一处理，优先查询本地数据库
- 设置页面增加滚动条
- 对话框底部间距优化
- 新增运行统计菜单（AP切换、本地查询、缓存使用、远程查询）

### v1.32
- DataStore 迁移设置存储，废弃 SharedPreferences
- 硬编码中文字符串迁移到 strings.xml
- 查询结果显示原始 JSON 字符串
- 关于页面增加"详细说明"按钮，打开项目介绍 HTML
- 主页各区域间距调整
